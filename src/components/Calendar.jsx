...
export default function Calendar() {
  ...
  const monthName = t.invitation.months[month - 1];
  const dows = t.calendar.dows;
  ...
  return (
    <section className="px-4 py-16 max-w-xl mx-auto text-center">
      ...
      >
        <p className="font-display text-xl text-mocha tracking-[0.25em] uppercase">{monthName} {year}</p>
        <div className="grid grid-cols-7 mt-5 text-[11px] font-medium text-gold uppercase tracking-wider">
          {dows.map((d) => <div key={d} className="py-2">{d}</div>)}
        </div>
        ...
        <div className="grid grid-cols-7 gap-y-1">
          {cells.map((d, i) => (
            <div key={i} className={
              ...
              (d === day ? 'bg-gold text-white font-semibold text-lg shadow-md' : d ? 'text-mocha/80 hover:bg-gold/10' : 'text-transparent pointer-events-none')
            }>
              {d || '·'}
              {d === day && <span className="absolute -top-1 -right-0.5 text-[11px] text-red-500 leading-none">❤</span>}
            </div>
          ))}
        </div>
      </motion.div>
